...
def predict(
    ...
    def predict(test_inputs: ndarray) -> Normal:
        ...
        # μt  +  Ktx (Kxx + Iσ²)⁻¹ (y  -  μx)
        mean = μt + torch.matmul(Sigma_inv_Kxt.T, y - μx)

        # Ktt  -  Ktx (Kxx + Iσ²)⁻¹ Kxt
        covariance = Ktt - torch.matmul(Kxt.T, Sigma_inv_Kxt)
        ...
        return Normal(torch.atleast_1d(mean.squeeze()), covariance)
